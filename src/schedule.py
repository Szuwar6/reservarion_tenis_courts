...
class Schedule:
    ...
    def show(self):
        sorted_reservations = sorted(self.list_of_reservations, key=lambda r: r.start_date)
    ...
    def count_sum_reservation(self, name, start_date):
        count = 0
        week = start_date.isocalendar()[1]
        year = start_date.year
        for reservation in self.list_of_reservations:
            if reservation.name == name and reservation.number_of_week == week and reservation.start_date.year == year:
                count += 1
        return count

    def check_if_court_is_reserved_start_date(self, start_date):
        sorted_reservations = sorted(self.list_of_reservations, key=lambda r: r.start_date)
        if len(sorted_reservations) == 1:
            if sorted_reservations[0].start_date <= start_date < sorted_reservations[0].end_date:
        ...
        while i < len(sorted_reservations):
            if sorted_reservations[i].start_date <= start_date < sorted_reservations[i].end_date and \
                    sorted_reservations[i].end_date + datetime.timedelta(minutes=30) > \
                    sorted_reservations[i + 1].start_date:
                start_date = sorted_reservations[i + 1].end_date
                i += 1
                continue
            if sorted_reservations[i].start_date <= start_date < sorted_reservations[i].end_date:
                return sorted_reservations[i].end_date
            i += 1
        return start_date

    def check_if_court_is_reserved_end_date(self, end_date):
        for reservation in self.list_of_reservations:
            if reservation.start_date < end_date <= reservation.end_date:
                # or end_date - datetime.timedelta(
                # minutes=30) == reservation.end_date:
                return False
    ...
    def cancel_reservation(self):
        ...
        for reservation in self.list_of_reservations:
            if reservation.name == name and reservation.start_date == start_date and start_date - datetime.timedelta(
                    minutes=60) > datetime.datetime.now():
            ...
            elif reservation.name == name and reservation.start_date == start_date and start_date - datetime.timedelta(
                    minutes=60) < datetime.datetime.now():
                return print("Reservations cannot be canceled less than 1 hour before the start.\n")
        print("Reservation not found.\n")

    def save_to_file_csv(self, filename, start_date, end_date):
        with open(filename, "w", newline="") as file:
            ...
            for reservation in self.list_of_reservations:
                if reservation.start_date >= start_date and reservation.end_date <= end_date:
                    writer.writerow([reservation.name, reservation.start_date, reservation.end_date])
            print(f"Schedule saved to {filename} in CSV format\n")

    def save_to_file_json(self, filename, start_date, end_date):
        reservations_json = []
        for reservation in self.list_of_reservations:
            if reservation.start_date >= start_date and reservation.end_date <= end_date:
                reservation_dict = {"name": reservation.name,
                                    "start_date": reservation.start_date.isoformat(),
                                    "end_date": reservation.end_date.isoformat()}
                reservations_json.append(reservation_dict)
        with open(filename, "w") as file:
            json.dump(reservations_json, file)
